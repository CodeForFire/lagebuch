...
public class EinsatzgebietSectionTests : IDisposable
{
    ...
    public void KartendatenStatus_reports_both_files_missing()
    {
        ...
        Assert.False(section.KartendatenGefunden);
        Assert.Contains("region.mbtiles", section.KartendatenStatus, StringComparison.Ordinal);
        Assert.Contains("region.dem", section.KartendatenStatus, StringComparison.Ordinal);
    }
    ...
    public void KartendatenStatus_names_only_the_missing_file_when_one_is_present()
    {
        ...
        Assert.False(section.KartendatenGefunden);
        Assert.DoesNotContain("region.mbtiles", section.KartendatenStatus, StringComparison.Ordinal);
        Assert.Contains("region.dem", section.KartendatenStatus, StringComparison.Ordinal);
    }
