...
public sealed partial class EtbViewModel : ObservableObject, IDisposable
{
    ...
    // Opens the create-task overlay pre-filled with an entry's text and anchored to its timestamp
    // (#88, #247); null where the host offers no task feature, which disables the "add & create
    // task" dock button and every row's create-task icon too.
    ...
    private void AddEntryAndCreateTask()
    {
        ...
        NewTo = null;
        _onChanged();
        _createTaskFromEntry?.Invoke(text, timestamp);
    }
    ...
    // Hidden entirely (not just disabled) when the host offers no task feature or the incident is
    // read-only/closed, since such a session can never save the resulting task anyway (#247).
    private bool CanCreateTaskFromRow => !IsReadOnly && _createTaskFromEntry is not null;
    ...
    private EtbEntryRow ToRow(EtbEntry e) =>
        new(e, BeginEdit, CanEdit, ShowHistory, CanCreateTaskFromRow ? CreateTaskFromRow : null);
}
...
public sealed class EtbEntryRow
{
        ...
        Action<EtbEntryRow>? createTask)
    {
        ...
        // Null (rather than merely disabled) hides the icon entirely where the host offers no task
        // feature or the incident is read-only/closed (#247) — see EtbViewModel.CanCreateTaskFromRow.
        CanCreateTask = createTask is not null;
