    ...
    public static IncidentSnapshot ToSnapshot(Incident incident)
    {
        ...
        return new IncidentSnapshot(
            ...
            incident.Forces.Select(f => new ForceUnitDto(
                ...
                f.Notes,
                f.OfficerCount,
                f.Edits.Select(x => new ForceUnitStrengthEditDto(x.PreviousOfficerCount, x.PreviousPersonnelCount, x.PreviousScbaCount, x.EditedBy, x.EditedAt, x.PreviousZugfuehrerCount)).ToList(),
                f.ZugfuehrerCount)).ToList(),
            ...
            incident.Buildings.Select(b => new BuildingDto(
                ...
                b.Ordinal,
                b.ApartmentLabels.ToDictionary(kv => kv.Key, kv => kv.Value),
                b.UndergroundFloorCount,
                b.ApartmentCounts.ToDictionary(kv => kv.Key.ToString(CultureInfo.InvariantCulture), kv => kv.Value))).ToList(),
    ...
    public static Incident FromSnapshot(IncidentSnapshot snapshot)
    {
        ...
        return Incident.Rehydrate(
            ...
            snapshot.Forces.Select(f => ForceUnit.Rehydrate(
                ...
                f.Notes,
                f.OfficerCount,
                f.Edits.Select(x => new ForceUnitStrengthEdit(x.PreviousOfficerCount, x.PreviousPersonnelCount, x.PreviousScbaCount, x.EditedBy, x.EditedAt, x.PreviousZugfuehrerCount)),
                f.ZugfuehrerCount)),
            ...
            snapshot.Buildings.Select(b => Building.Rehydrate(
                ...
                b.ApartmentLabels,
                b.UndergroundFloorCount,
                b.ApartmentCounts?.ToDictionary(kv => int.Parse(kv.Key, CultureInfo.InvariantCulture), kv => kv.Value))),
